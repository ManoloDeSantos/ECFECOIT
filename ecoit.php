<?php include("./template/header.php"); ?>

      <div class="jumbotron text-center">
        <h1 class="display-3">Bienvenue sur le site écologique de formation ECOIT</h1>
        <p class="lead">Apprenez le développement Web en respectant l’environnement.</p>
        <hr class="my-2">
                <h2>ECOIT Le project de formations numériques Eco Web</h2>
        <br/>
         <img width="500" src="image/prog.jpg" class="img-thumbnail rounded mx-auto d-block" />
                 <br/>
        <p class="lead">
          <a class="btn btn-primary btn-lg" href="ToutFormations.php" role="button">Voir Toutes les formations</a>
        </p>
      </div>
<?php include("template/footer.php"); ?>